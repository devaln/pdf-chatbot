...
def extract_all_tables(pdf_path, scanned_mode=False, llm=None):
    ...
    table_texts = [f"Table {i+1}:\n{df.to_csv(index=False)}" for i, df in enumerate(dfs)]
    table_texts.append("LLM-Structured Tables:\n" + llm_csv)
    return "\n\n".join(table_texts), text, dfs

@st.cache_resource(show_spinner=False)
def load_and_index(files, scanned_mode=False):
    all_docs = []
    with tempfile.TemporaryDirectory() as td:
        llm = ChatOllama(model=OLLAMA_LLM_MODEL, base_url=OLLAMA_BASE_URL, request_timeout=300, stream=True)
        for file in files:
            path = os.path.join(td, file.name)
            with open(path, "wb") as f:
                f.write(file.getbuffer())
            ...
            try:
                ...
                table_text, raw_text, dfs = extract_all_tables(path, scanned_mode, llm)
                if not (table_text or raw_text):
                    st.warning(f"⚠ Skipping {file.name} — no valid content extracted.")
                    continue

                all_docs.append(Document(page_content=table_text + "\n" + raw_text, metadata={"source": file.name}))
                all_docs.extend(chunk_table_rows(dfs, file.name))
...
def get_chat_chain(vs):
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a table analysis expert. Only use the relevant table rows and paragraph text to answer."),
        ("human", "{context}\n\nQuestion: {question}")
    ])
    llm = ChatOllama(model=OLLAMA_LLM_MODEL, base_url=OLLAMA_BASE_URL, temperature=0.1, stream=True)
    return {"context": vs.as_retriever(), "question": RunnablePassthrough()} | prompt | llm | StrOutputParser()
